...
//            throw new IllegalArgumentException("Bean name already present " + beanRecord.getName());
    ...
    public void dumpBeans(PrintWriter out) {
        ...
        out.println("Dumping all beans within ApplicationContext:\n-------------------------------------------");
        for (BeanRecord beanRecord : beansToDump.values()) {
            if (isConfigurationClass(beanRecord.getRuntimeClass())) continue;
    ...
    public String dumpBeanRecord(BeanRecord beanRecord) {
        String text = "\n<bean";
        if (!beanRecord.isAnonymous() && !beanRecord.getName().equals(beanRecord.getDefinitionClass().getName())) {
